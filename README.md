...
基于 hydros-engine-executor MCP 服务的 Claude Code Skill，编排 MCP 工具调用，引导用户完成水力仿真场景查询、任务运行、进度跟踪和结果分析的完整流程；进入场景建模元数据、拓扑和 `objects.yaml` 相关流程前，还要求 `hydros-engine-mdm` 已完成配置并可用。
...
2. 读取场景配置，补充默认参数、预置事件和简要拓扑信息；其中场景建模元数据链路要求 `hydros-engine-mdm` 已配置可用
