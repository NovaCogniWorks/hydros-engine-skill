...
| `get_task_step` | 常规轮询当前仿真步数 |
| `get_task_status` | 仅在仿真出错时查询全量状态和异常信息记录 |
...
3. 创建成功后持续调用 `get_task_step` 轮询当前步数，并读取 `received_hydro_events` 判断运行中是否有事件发生
4. 自动向用户播报文本进度条、当前步数和新增事件摘要；只有仿真出错时才调用 `get_task_status` 记录全量状态和异常信息
...
4. **数据准确性分析：**
   - 水位、流量、闸门开度异常检测
   - 沿程水头损失和关键断面变化分析
   - 关键事件、故障影响和运行风险提示；完整事件记录仅在报告、复盘、异常排查或用户明确查看事件时查询
