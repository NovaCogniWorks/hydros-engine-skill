...
- 生成正式报告、事件复盘或运行过程记录时，如已调用 `get_simulation_scenario_events(biz_scene_instance_id)` 且拿到有效事件，必须把工况事件显式写入 HTML/Markdown 正文；不能只在摘要里笼统写“当前工况”“已注入事件”，也不能把原始 JSON 只藏在 payload 里不渲染。
- 工况事件在正式报告里的最小展示字段至少包括：事件名称/类型、注入步号 `auto_schedule_at_step`、可推导的注入时间、优先级、事件描述；如还能拿到作用对象、关联测点或时序项，也应补充展示。
- 若事件时间需要推导，优先使用 `biz_start_time + auto_schedule_at_step * sim_step_size`；若缺少起始时间或步长，则明确写“时间无法推导”，不要伪造自然时间。
- 对 `get_simulation_scenario_events` 的结果解析要兼容 MCP 包裹差异：优先检查 `result.structuredContent.result`，其次检查 `result.content[0].text` 里的 JSON 文本，再回退到 `result` 本体；不要假设事件数据一定平铺在单一层级。
...
- 只要本轮拉取了完整工况事件，HTML 主页面必须有独立事件区块，Markdown 正文也必须有对应章节；至少要让用户直接看到事件摘要和事件列表，不能仅在“摘要 bullets”里一笔带过。
- 正式报告脚本需要把整理后的 `scenarioEvents` 一类结构写入 payload，并保证模板里存在明确的渲染入口；“payload 已包含事件 JSON，但页面没有展示”仍视为报告不完整。
